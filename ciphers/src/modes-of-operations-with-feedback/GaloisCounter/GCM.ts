...
class GCM extends CipherCore {
    ...
    constructor(key: any, authData: any) {
        super();
        this.key = key;
    ...
    encryptFunction(input: string) {
        return this.bitsXor(input, this.key);
    }

    multH(input: any): string {
        return (input & this.H).toString();
    }
     ...
     * cipertext
     */
    ghash(H:string, A: string, C: string) {
